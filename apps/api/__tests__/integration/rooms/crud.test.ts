...
describe('Rooms CRUD', () => {
  ...
  const token = signAccessToken({ sub: 'test-user-id', email: 't@t.com', role: 'TENANT', isVerified: true });
  ...
  describe('POST /api/rooms/tenant/properties/:propertyId/rooms', () => {
    ...
    it('should return 403 when trying to modify another tenant property', async () => {
      const otherToken = signAccessToken({ sub: 'other-user-id', email: 'o@t.com', role: 'TENANT', isVerified: true });
      const res = await request(app)
