...
export default function RegisterPage() {
  const [success, setSuccess] = useState(false), { cooldown, setCooldown } = useRegisterCooldown();
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center p-4"><Card className="w-full max-w-md">
      ...
      <CardContent><form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <RegisterFormFields register={form.register} errors={form.formState.errors} isSubmitting={isSubmitting} /><RegisterFormActions isSubmitting={isSubmitting} />
      </form></CardContent>
    </Card></div>
  );
}
