...
const verifyEmailFn = async (
  ...
  try {
    ...
    if (res.ok && data.success)
      await fetch(`${clientEnv.apiBaseUrl}/auth/logout`, {
        method: 'POST',
        credentials: 'include',
      }).catch(() => {});
  } catch {
    if (isMounted.current) setMessage('Server error');
  } finally {
  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center p-4">
      <div className="text-center space-y-3">
        ...
        <p className="text-muted-foreground text-sm">Memverifikasi email Anda...</p>
      </div>
    </div>
  );
}
      ...
      {success ? (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Sesi Anda sebelumnya telah diakhiri demi keamanan. Silakan masuk kembali menggunakan
            email baru Anda.
          </p>
          <Link
            href={role === 'TENANT' ? '/tenant/login' : '/login'}
