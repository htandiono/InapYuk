...
export async function UnverifiedBanner() {
  ...
  let isVerified = true;
  try { isVerified = (decodeJwt(token) as unknown as JwtPayload).isVerified; } catch {}
  ...
  return (
    <div className="bg-amber-100 p-3 text-center text-sm font-medium text-amber-800">
      ...
      <a href="/resend-verification" className="underline hover:text-amber-900 font-bold">Kirim ulang email</a>
    </div>
  );
}
