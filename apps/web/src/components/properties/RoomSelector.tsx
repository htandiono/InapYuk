...
function RoomCard({ room, selectedId, onSelect, night, onLightbox }: { room: Room, selectedId: string, onSelect: (id: string) => void, night?: { date?: string, price: number, isAvailable: boolean } | null, onLightbox: (id: string) => void }) {
  const isSelected = room.id === selectedId;
  const cls = isSelected ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'border-border bg-card hover:border-primary/50 hover:shadow-sm';
  return (
    <button onClick={() => onSelect(room.id)} className={`text-left flex flex-col rounded-2xl border overflow-hidden transition-all ${cls}`}>
...
function RoomImage({ room, onLightbox }: { room: Room, onLightbox: (id: string) => void }) {
  if (!room.images || room.images.length === 0) {
    return <div className="relative w-full aspect-video bg-muted/50 flex flex-col items-center justify-center text-muted-foreground"><ImageIcon className="h-8 w-8 mb-2 opacity-50" /><span className="text-xs font-medium opacity-70">Tidak ada foto</span></div>;
  }
  return (
    <div className="relative w-full aspect-video bg-muted group cursor-pointer" onClick={(e) => { e.preventDefault(); e.stopPropagation(); onLightbox(room.id); }}>
      <Image src={room.images[0].url} alt={room.name} fill className="object-cover transition-transform duration-300 group-hover:scale-105" sizes="(max-width: 640px) 100vw, 50vw" />
      {room.images.length > 1 && <div className="absolute bottom-2 right-2 bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded-md backdrop-blur-sm">1 / {room.images.length}</div>}
    </div>
  );
}

function RoomDetails({ room, isSelected, night }: { room: Room, isSelected: boolean, night?: { date?: string, price: number, isAvailable: boolean } | null }) {
  return (
...
function RoomMeta({ room, isSelected, night }: { room: Room, isSelected: boolean, night?: { date?: string, price: number, isAvailable: boolean } | null }) {
...
function RoomPrice({ room, isSelected, night }: { room: Room, isSelected: boolean, night?: { date?: string, price: number, isAvailable: boolean } | null }) {
  ...
  const suffix = isSelected && night && night.date ? ` / mlm (${new Date(night.date as string).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })})` : ' / mlm';
  return (
