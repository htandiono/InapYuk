...
export function LightboxDialog({ isOpen, onClose, images, currentIndex, setCurrentIndex, altPrefix }: LightboxDialogProps) {
  return (
...
function LightboxBtn({ dir, onClick }: { dir: 'left' | 'right'; onClick: (e: React.MouseEvent) => void; }) {
  const isLeft = dir === 'left', posClass = isLeft ? 'left-2 md:left-6' : 'right-2 md:right-6', Icon = isLeft ? ChevronLeft : ChevronRight, label = isLeft ? 'Sebelumnya' : 'Selanjutnya';
  return (
    <button onClick={onClick} className={`absolute ${posClass} z-50 p-2 md:p-3 rounded-full bg-black/50 hover:bg-black/80 transition-colors text-white backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/50`}>
      <Icon className="w-6 h-6 md:w-8 md:h-8" /><span className="sr-only">{label}</span>
    </button>
  );
}
  ...
  return (
    <div className="absolute bottom-4 left-0 right-0 h-16 md:h-20 px-4 flex justify-center gap-2 overflow-x-auto z-50 scrollbar-hide">
      <div className="flex gap-2 mx-auto">
        {images.map((img, idx) => <ThumbnailItem key={idx} img={img} idx={idx} currentIndex={currentIndex} setCurrentIndex={setCurrentIndex} />)}
      </div>
    </div>
  );
}
