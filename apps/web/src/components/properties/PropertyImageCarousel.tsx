...
function CarouselArrows({
  ...
  current: number;
  max: number;
  onScrollTo: (i: number, e: MouseEvent) => void;
}) {
  return (
    <div className="absolute inset-0 flex items-center justify-between p-2 opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-300 pointer-events-none">
      <ArrowButton
        dir="left"
        onClick={(e) => onScrollTo(Math.max(0, current - 1), e)}
        disabled={current === 0}
      />
      <ArrowButton
        dir="right"
        onClick={(e) => onScrollTo(Math.min(max, current + 1), e)}
  ...
  return (
    <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 pointer-events-none">
      {Array.from({ length: total }).map((_, i) => (
        <div
          key={i}
          className={`w-1.5 h-1.5 rounded-full transition-all duration-300 shadow-sm ${i === current ? 'bg-white scale-125' : 'bg-white/60'}`}
        />
      ))}
    </div>
  );
    ...
    <div className={`${aspectClass} w-full bg-muted relative overflow-hidden group/carousel`}>
      ...
      >
        {imageUrls.map((url: string, i: number) => (
          <div key={i} className="shrink-0 w-full h-full relative snap-center">
            <Image
              src={url}
              alt={`${name} - Foto ${i + 1}`}
              fill
