  ...
  return (
    <div onClick={() => !isPast && night.isAvailable && onSelectDate()} className={wrapCls}>
      <div className={dayCls}>
        <span className={txtCls}>{dateStr}</span>
      </div>
      {!isPast && (
    ...
    <div className="grid grid-cols-7 gap-x-0 gap-y-2">
      ...
      {nights.map((night, i) => (
        <NightCell
          key={i}
          night={night}
          isPast={new Date(night.date) < today}
          isSelected={selStr === night.date.split('T')[0]}
          onSelectDate={() =>
            onSelectDate(selStr === night.date.split('T')[0] ? null : night.date.split('T')[0])
          }
