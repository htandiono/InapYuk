...
type CDProps = { isOpen: boolean; filtered: City[]; onSelect: (city: string) => void; };
export function CityDropdown({ isOpen, filtered, onSelect }: CDProps) {
  if (!isOpen) return null;
  return (
    <div className="absolute top-full left-0 mt-2 w-full sm:min-w-[280px] max-h-60 overflow-y-auto rounded-xl border bg-background/95 backdrop-blur-xl shadow-xl z-50 p-1.5">
      {filtered.length === 0 ? (
...
type CIProps = { term: string; setTerm: (v: string) => void; setCity: (v: string) => void; setIsOpen: (v: boolean) => void; loading: boolean; isOpen: boolean; filtered: City[]; };
export function CityInput({ term, setTerm, setCity, setIsOpen, loading, isOpen, filtered }: CIProps) {
  return (
    <div className="flex-1 w-full relative">
      ...
      <div className="relative">
        <input required placeholder={loading ? 'Memuat...' : 'Ketik kota'} value={term} onChange={(e) => { setTerm(e.target.value); setCity(e.target.value); setIsOpen(true); }} onFocus={() => setIsOpen(true)} onBlur={() => setTimeout(() => setIsOpen(false), 200)} className="w-full rounded-lg bg-muted/30 px-4 py-3 text-sm border-none focus:ring-2 focus:ring-primary outline-none" />
