...
export function DateInput({
  ...
  return (
    <div className="flex-1 w-full">
      <label className="mb-1.5 block text-xs font-semibold text-muted-foreground uppercase px-2">
        {label}
      </label>
      <input
        required
...
export function DateFields({
  ...
  const minOut = checkIn
    ...
    : todayStr;
  const maxOut = checkIn
    ? new Date(new Date(checkIn).getTime() + 30 * 86400000).toISOString().split('T')[0]
    : maxYear;
  return (
    <>
      <DateInput
