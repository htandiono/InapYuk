...
function TenantAvatarLink({ displayName, initial }: { displayName: string; initial: string }) {
  return (
    <Link
      href="/tenant/profile"
      className="flex items-center gap-3 hover:bg-muted/50 px-3 py-1.5 rounded-lg transition-colors group"
    >
      ...
      <div className="hidden md:flex flex-col items-start min-w-0">
        <span className="text-sm font-medium leading-none text-foreground truncate max-w-30 lg:max-w-50">
          Halo, {displayName}
        </span>
      </div>
    </Link>
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-x-4 border-b border-border/40 bg-background px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-1 gap-x-2 self-stretch lg:gap-x-4 justify-end items-center">
        ...
        <LogoutButton
          variant="ghost"
          className="hidden md:flex px-4 h-9 text-sm font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
        />
        <LogoutButton
          variant="ghost"
          iconOnly
          className="md:hidden text-muted-foreground hover:text-destructive hover:bg-destructive/10 h-9 w-9 p-0 rounded-full flex items-center justify-center"
        />
      </div>
    </header>
  );
