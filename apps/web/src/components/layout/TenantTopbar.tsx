...
export function TenantTopbar() {
  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-x-4 border-b border-border/40 bg-background px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
      {/* Mobile Brand (Shown only on small screens when sidebar is hidden) */}
      ...
      <div className="flex flex-1 gap-x-2 self-stretch lg:gap-x-4 justify-end items-center">
        {/* Profile Link natively integrated into navbar */}
        <Link
          href="/tenant/profile"
        ...
        {/* Logout button blending natively */}
        <LogoutButton
          variant="ghost"
          className="hidden sm:flex px-4 h-9 text-sm font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
        />

        {/* Mobile icon-only logout */}
        <LogoutButton
          variant="ghost"
          className="sm:hidden text-muted-foreground hover:text-destructive hover:bg-destructive/10 px-2 h-9"
        />
      </div>
    </header>
  );
