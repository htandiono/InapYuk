...
function useRooms(propertyId: string, page: number) {
  ...
  const fetchRooms = useCallback(
    async (p: number) => {
      ...
      try {
        ...
        setState({
          rooms: json.data.items || json.data,
  ...
  useEffect(() => {
    void Promise.resolve().then(() => fetchRooms(page));
  }, [page, fetchRooms]);
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center border rounded-xl border-dashed bg-white">
      ...
      <p className="text-muted-foreground mt-1 max-w-sm text-sm">
        Tambahkan jenis kamar untuk properti ini agar dapat mulai disewakan.
      </p>
    </div>
  );
}
  ...
  if (loading)
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-52.5 rounded-xl bg-muted/50 animate-pulse" />
        ))}
      </div>
    );
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="w-[calc(100%-2rem)] sm:w-full sm:max-w-2xl min-w-[320px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Kamar Baru</DialogTitle>
        </DialogHeader>
...
export default function RoomList({ propertyId }: { propertyId: string }) {
  const st = useRoomListState(),
    { rooms, totalPages, loading, fetchRooms } = useRooms(propertyId, st.page),
    acts = useRoomActions(fetchRooms, st.page);
  return (
    <div className="space-y-6">
      <RoomListHeader setOpen={(c) => st.set([st.page, c, st.edit])} />
      <CreateRoomDialog
        open={st.isCreate}
        setOpen={(c) => st.set([st.page, c, st.edit])}
        propertyId={propertyId}
        onDone={() => {
          st.set([st.page, false, st.edit]);
          fetchRooms(st.page);
        }}
      />
      <RoomListGrid
        loading={loading}
        rooms={rooms}
        setEditing={(e) => st.set([st.page, st.isCreate, e])}
        handleDelete={acts.setDeletingId}
        onManageAvailability={st.setAvailabilityRoomId}
        onManagePeakSeason={st.setPeakSeasonRoomId}
      />
      <PaginationControls
        page={st.page}
        ...
        setPage={(p) =>
          st.set([
            typeof p === 'function' ? (p as (n: number) => number)(st.page) : p,
            st.isCreate,
            st.edit,
          ])
        }
      ...
      <EditRoomDialog
        r={st.edit}
        setR={(e) => st.set([st.page, st.isCreate, e])}
        propertyId={propertyId}
        onDone={() => {
          st.set([st.page, st.isCreate, null]);
          fetchRooms(st.page);
        }}
      />
      <DeleteRoomDialog {...acts} />
      <AvailabilityDialog
        roomId={st.availabilityRoomId}
        totalUnits={rooms.find((r) => r.id === st.availabilityRoomId)?.totalUnits}
        onClose={() => st.setAvailabilityRoomId(null)}
      />
