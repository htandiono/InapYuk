...
interface PropertyFormFieldsProps {
  ...
  errors: { name?: { message?: string }; categoryId?: { message?: string }; description?: { message?: string } };
  loading: boolean; nameValue: string; descriptionValue: string;
  onNameChange: (val: string) => void; onDescChange: (val: string) => void;
  categories: { id: string; name: string }[];
}

function NameField({ control, errors, loading, nameValue, onNameChange }: { control: Control<PropertyFormValues>; errors: { name?: { message?: string } }; loading: boolean; nameValue: string; onNameChange: (val: string) => void; }) {
  return (
    <div>
      <label className="block text-sm mb-1">Nama Properti</label>
      <Controller name="name" control={control} render={({ field }) => (
        <Input {...field} maxLength={100} disabled={loading} className={errors.name ? 'border-destructive' : ''} />
      )} />
...
function CategoryField({ control, errors, loading, categories }: { control: Control<PropertyFormValues>; errors: { categoryId?: { message?: string } }; loading: boolean; categories: { id: string; name: string }[]; }) {
  return (
    <div>
...
function DescriptionField({ control, errors, loading, descriptionValue, onDescChange }: { control: Control<PropertyFormValues>; errors: { description?: { message?: string } }; loading: boolean; descriptionValue: string; onDescChange: (val: string) => void; }) {
  return (
    <div>
      <label className="block text-sm mb-1">Deskripsi</label>
      <Controller name="description" control={control} render={({ field }) => (
        <textarea {...field} maxLength={1000} disabled={loading} onChange={(e) => { onDescChange(e.target.value); field.onChange(e); }} className={`w-full min-h-20 rounded border border-input bg-transparent px-3 py-2 shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring ${errors.description ? 'border-destructive' : ''}`} />
      )} />
      <div className="flex justify-between text-xs mt-1">
...
export function PropertyFormFields({ control, errors, loading, nameValue, descriptionValue, onNameChange, onDescChange, categories }: PropertyFormFieldsProps) {
  return (
    <>
      <NameField control={control} errors={errors} loading={loading} nameValue={nameValue} onNameChange={onNameChange} />
      <CategoryField control={control} errors={errors} loading={loading} categories={categories} />
      <DescriptionField control={control} errors={errors} loading={loading} descriptionValue={descriptionValue} onDescChange={onDescChange} />
    </>
  );
}
