    ...
    <div className="flex sm:hidden justify-end">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuItem asChild><Link href={`/tenant/properties/${p.id}/rooms`} className="cursor-pointer flex items-center gap-2 w-full"><Settings className="h-4 w-4" /><span>Kelola Kamar</span></Link></DropdownMenuItem>
          <DropdownMenuItem onClick={() => onEdit(p)} className="cursor-pointer flex items-center gap-2 w-full"><Edit className="h-4 w-4" /><span>Edit Properti</span></DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => onDelete(p.id)} className="text-destructive focus:text-destructive cursor-pointer flex items-center gap-2 w-full"><Trash2 className="h-4 w-4" /><span>Hapus Properti</span></DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
