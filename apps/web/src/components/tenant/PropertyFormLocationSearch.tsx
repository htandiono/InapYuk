...
async function fetchSuggestions(query: string, provinceId: string) {
  ...
  if (p) url.searchParams.append('province', p);
...
function useSuggestionsLocal(searchQuery: string, selectedProvinceId: string) {
  ...
  const [showSuggestionsLocal, setShowSuggestionsLocal] = useState(false),
    [isSearchingLocal, setIsSearchingLocal] = useState(false);
  useEffect(() => {
    if (searchQuery.length < 3) return;
    const t = setTimeout(() => {
      setIsSearchingLocal(true);
      fetchSuggestions(searchQuery, selectedProvinceId)
        .then((data) => {
          if (data) {
            setSuggestionsLocal(data);
...
function SearchInputSection({
  ...
  searchQuery: string;
  setSearchQuery: (q: string) => void;
  setSuggestionsLocal: (s: { formatted: string; lat: number; lng: number }[]) => void;
  setShowSuggestionsLocal: (v: boolean) => void;
  loading: boolean;
  selectedProvinceId: string;
  isSearchingLocal: boolean;
  suggestionsLocal: { formatted: string; lat: number; lng: number }[];
}) {
  return (
...
function MapSection({
  ...
  selectedGeo: { lat: number; lng: number };
  handleMarkerDrag: (lat: number, lng: number) => void;
}) {
  return (
    <div className="mt-3">
...
export function LocationSearch({
  ...
  const {
    ...
    setShowSuggestionsLocal,
    isSearchingLocal,
  } = useSuggestionsLocal(searchQuery, selectedProvinceId);
  return (
    <div className="relative z-10 pt-2">
      ...
      <SearchInputSection
        ...
        setSuggestionsLocal={setSuggestionsLocal}
        setShowSuggestionsLocal={setShowSuggestionsLocal}
        loading={loading}
