...
function usePropertyFormLogic(initialData?: PropertyFormInitData, onSuccess?: () => void) {
  ...
  const imgHook = usePropertyFormImages(initialData);
  ...
  const [searchQuery, setSearchQuery] = useState(''),
    ...
    [selectedGeo, setSelectedGeo] = useState<{ lat: number; lng: number } | null>(
  ...
  const handleMarkerDrag = async (lat: number, lng: number) => {
    ...
    await handleReverseGeo(lat, lng, (field, value, options) =>
      form.setValue(
        field as keyof PropertyFormValues,
    ...
    try {
      await submitPropertyForm(
        buildFormData(
          values,
          imgHook.files,
          imgHook.deletedImages,
          imgHook.mainImageId,
          imgHook.mainImageIndex,
          initialData,
        ),
        initialData,
      );
  ...
  return (
    <>
      <PropertyFormFields
        ...
        errors={logic.form.formState.errors}
