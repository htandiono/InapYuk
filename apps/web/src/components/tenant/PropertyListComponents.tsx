...
export function EditDialog({ p, setP, onDone }: { p: Property | null; setP: (p: Property | null) => void; onDone?: () => void; }) {
  ...
  return (
    <Dialog open={!!p} onOpenChange={(o) => !o && setP(null)}>
      <EditDialogContent setP={setP} onDone={onDone} fullProp={fullProp} loadingFull={loadingFull} />
    </Dialog>
  );
}
...
export function PropertyListHeader({ setIsCreateOpen }: { setIsCreateOpen: (v: boolean) => void }) {
  return (
    <div className="flex justify-end mb-6">
      <button onClick={() => setIsCreateOpen(true)} className="shadow-sm rounded-full px-6 py-2 bg-primary text-white font-medium hover:bg-primary/90 transition-colors">Tambah Properti</button>
    </div>
  );
}
...
function TableBody({ loading, properties, setEditingProperty, setDeletingId }: { loading: boolean; properties: Property[]; setEditingProperty: (p: Property) => void; setDeletingId: (id: string) => void; }) {
  return (
    <tbody>
  ...
  return (
    <DialogContent className="sm:max-w-md">
      ...
      <div className="flex justify-end gap-3 mt-4">
        <button onClick={() => setDeletingId(null)} disabled={isDeleting} className="px-4 py-2 rounded-md border border-border hover:bg-muted transition-colors">Batal</button>
        <button onClick={confirmDelete} disabled={isDeleting} className="px-4 py-2 rounded-md bg-destructive text-white hover:bg-destructive/90 transition-colors">{isDeleting ? 'Menghapus...' : 'Ya, Hapus'}</button>
      </div>
    </DialogContent>
  );
}

export function DeleteConfirmation({ deletingId, isDeleting, setDeletingId, confirmDelete }: { deletingId: string | null; isDeleting: boolean; setDeletingId: (id: string | null) => void; confirmDelete: () => void; }) {
  return (
    <Dialog open={!!deletingId} onOpenChange={(open) => !open && !isDeleting && setDeletingId(null)}>
      <DeleteDialogContent isDeleting={isDeleting} setDeletingId={setDeletingId} confirmDelete={confirmDelete} />
    </Dialog>
  );
}
