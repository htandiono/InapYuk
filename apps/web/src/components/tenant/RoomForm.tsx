...
function RoomFormActions({ onCancel, isSubmitting }: { onCancel?: () => void; isSubmitting: boolean; }) {
  return (
...
export default function RoomForm({ propertyId, initialData, onSuccess, onCancel }: { propertyId: string; initialData?: RoomFormInitData; onSuccess?: () => void; onCancel?: () => void; }) {
  const state = useRoomForm(propertyId, initialData, onSuccess);
  const { handleSubmit, formState: { isSubmitting } } = state.form;
  return (
    <form onSubmit={handleSubmit((d) => submitForm(d, state))} className="space-y-4">
      <RoomFormImages state={state} previews={initialData?.images || []} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleImageFiles(e, state, initialData?.images?.length || 0)} />
