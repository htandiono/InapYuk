...
export function PropertyFormImageGrid({
  existingImages,
  files,
  mainImageId,
  mainImageIndex,
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-4 mb-2">
        {existingImages.map((img) => (
          <ExistingImageCell
            ...
            onSetMain={onSetMainImageExisting}
        ...
        {files.map((file, i) => (
          <NewFileCell
            ...
            onSetMain={onSetMainImageNew}
          />
        ))}
        {totalSlots < 10 && <UploadButton loading={loading} onChange={handleFileChange} />}
      </div>
    </div>
  );
}
