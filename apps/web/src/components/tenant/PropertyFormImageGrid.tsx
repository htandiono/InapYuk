...
interface GridProps {
  ...
  onSetMainImageExisting: (id: string) => void;
  onSetMainImageNew: (index: number) => void;
  ...
  return (
    <>
      ...
      <p className="text-xs text-muted-foreground mb-3">
        Format: JPG/PNG · 50KB–5MB · Min. 800×600px · Maks. 10 foto
      </p>
    </>
  );
}
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-4 mb-2">
        {props.existingImages.map((img) => (
          <ExistingImageCell
            ...
            onSetMain={props.onSetMainImageExisting}
            onDelete={props.onDeleteExisting}
          />
        ))}
        {props.files.map((file, i) => (
          <NewFileCell
            ...
            file={file}
            index={i}
            isMain={props.mainImageIndex === i}
            removeNewFile={props.removeNewFile}
            onSetMain={props.onSetMainImageNew}
          />
        ))}
        {props.totalSlots < 10 && (
          <UploadButton loading={props.loading} onChange={props.handleFileChange} />
        )}
      </div>
    </div>
  );
