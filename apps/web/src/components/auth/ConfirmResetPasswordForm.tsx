...
async function submitReset(
  ...
  setError(null);
  try {
    const res = await api.post<{ role: string }>('/auth/password/confirm', {
      token,
...
function useConfirmToken(token: string | null) {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    api
      .get(`/auth/password/check?token=${token}`)
      .then(() => {
        if (mounted) setTokenStatus('valid');
      })
      .catch(() => {
      ...
      <CardContent>
        <FormContent
          ...
          onSubmit={handleSubmit(
            (d) => token && submitReset(token, d, router, setServerError, setIsSubmitting),
          )}
